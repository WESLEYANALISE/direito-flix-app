...
const CourseDetail = () => {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      ...
      } = await supabase.from('user_course_favorites').select('*').eq('course_id', course.id).eq('user_id', userId).maybeSingle();
      if (existingFavorite) {
        // Remove from favorites
        await supabase.from('user_course_favorites').delete().eq('course_id', course.id).eq('user_id', userId);
        setIsFavorite(false);
        toast({
        ...
        // Add to favorites
        await supabase.from('user_course_favorites').insert({
          course_id: course.id,
          user_id: userId
        });
        setIsFavorite(true);
  ...
  const toggleCompleted = async () => {
    ...
    try {
      ...
      } = await supabase.from('user_course_completed').select('*').eq('course_id', course.id).eq('user_id', userId).maybeSingle();
      if (existingCompleted) {
        // Remove from completed
        await supabase.from('user_course_completed').delete().eq('course_id', course.id).eq('user_id', userId);
        ...
        // Add to completed
        await supabase.from('user_course_completed').insert({
          course_id: course.id,
          user_id: userId
        });
        setIsCompleted(true);
  ...
  if (showWebView && course) {
    return <div className="fixed inset-0 bg-black flex flex-col z-50">
        <div className="bg-black p-4 flex items-center px-[15px] py-[6px]">
          ...
          <h2 className="ml-4 font-semibold text-sm">{course.materia}</h2>
        </div>
        <iframe src={course.link} title={course.materia} className="flex-1 w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
      </div>;
  }
  const startCourse = () => {
    setShowWebView(true);
  ...
  return <div className="pb-12">
      {course && <>
          <button onClick={() => navigate("/")} className="flex items-center text-netflix-secondary hover:text-netflix-accent mb-4">
          ...
          <div className="relative h-[40vh] mb-8 overflow-hidden rounded-lg">
            ...
            <img src={course.capa} alt={course.materia} className="w-full h-full object-cover" />
            <div className="absolute top-4 right-4 z-20 flex gap-2">
              <button onClick={toggleCompleted} className="bg-black bg-opacity-60 p-2 rounded-full">
                <CheckCircle size={24} className={`${isCompleted ? 'fill-green-500 text-green-500' : 'text-white'}`} />
              </button>
              <button onClick={toggleFavorite} className="bg-black bg-opacity-60 p-2 rounded-full">
                <Heart size={24} className={`${isFavorite ? 'fill-netflix-accent text-netflix-accent' : 'text-white'}`} />
              </button>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              ...
              <button onClick={toggleCompleted} className={`flex items-center gap-2 px-4 py-2 rounded ${isCompleted ? 'bg-green-600 text-white' : 'bg-gray-700 text-white'}`}>
