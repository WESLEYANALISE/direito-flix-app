...
const AreaShowcase = ({ area, courseCount, imageUrl }: AreaShowcaseProps) => {
  // Default background images for different areas
  ...
  const bgImage = imageUrl || defaultImages[area] || "https://images.unsplash.com/photo-1589994965851-a7f54d2274b9";
  ...
  return (
    <Link to={`/area/${encodeURIComponent(area)}`} className="block">
      <div className="relative rounded-lg overflow-hidden h-40 group">
        ...
        <img 
          src={`${bgImage}?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80`} 
          alt={area} 
